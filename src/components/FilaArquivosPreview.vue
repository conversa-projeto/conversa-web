<template>
  <div class="preview-grid mb-2 flex max-h-48 min-w-0 flex-wrap items-start gap-1 overflow-y-auto rounded border border-surface-300 bg-surface-50 p-2">
    <div
      v-for="arq in arquivos"
      :key="arq.id"
      class="preview-card flex min-h-[68px] min-w-0 w-full items-center gap-2 rounded border border-surface-300 bg-surface-100 px-2 py-2 text-xs text-surface-700 shadow-sm sm:w-[400px] sm:max-w-[400px] sm:px-3"
    >
      <button
        v-if="arq.isAudio"
      ...
      <button class="preview-remove ml-1 h-7 w-7 shrink-0 rounded text-surface-400 hover:bg-danger-50 hover:text-danger-500 dark:hover:bg-danger-900 dark:hover:text-danger-400" @click="emit('remover', arq.id)" title="Remover">&times;</button>
    </div>
  </div>
</template>
