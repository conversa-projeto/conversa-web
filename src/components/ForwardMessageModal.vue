<template>
  <div v-if="aberta && mensagem" class="fixed inset-0 z-30 flex items-center justify-center bg-slate-900/50 p-4">
    <div class="w-full max-w-lg rounded-2xl bg-white p-4 shadow-2xl">
      <div class="mb-4 flex items-start justify-between gap-3">
        <div>
          <h3 class="select-none text-lg font-semibold text-slate-800">Encaminhar mensagem</h3>
          <p class="select-none text-sm text-slate-500">Escolha uma conversa ou contato para receber esta mensagem.</p>
        </div>
        <button class="h-10 w-10 shrink-0 rounded-full text-slate-400 transition hover:bg-slate-100 hover:text-slate-600" @click="emit('close')">
      ...
      <div class="mb-4 rounded-xl border border-blue-100 bg-blue-50 px-3 py-2">
        <span class="block select-none text-xs font-semibold text-blue-600">Mensagem selecionada</span>
        <p class="mt-1 truncate text-sm text-slate-700">{{ resumoMensagem(mensagem) }}</p>
      </div>
      ...
      <label class="mb-3 block text-sm text-slate-700">
        ...
        <input
          v-model="busca"
          type="text"
          class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-blue-500"
          placeholder="Nome da conversa ou contato"
        />
      </label>

      <div class="max-h-[360px] overflow-y-auto rounded-xl border border-slate-200">
        <div v-if="destinosFiltrados.length === 0" class="px-4 py-6 text-center text-sm text-slate-500">
        ...
        <button
          ...
          :key="destino.key"
          class="flex w-full items-center justify-between gap-3 border-b border-slate-100 px-4 py-3 text-left transition last:border-b-0 hover:bg-slate-50"
          @click="selecionarDestino(destino)"
        >
          <div class="min-w-0">
            <p class="truncate text-sm font-medium text-slate-800">{{ destino.titulo }}</p>
            <p class="truncate text-xs text-slate-500">{{ destino.descricao }}</p>
          </div>
          <span
            class="rounded-full px-2 py-1 text-[11px] font-semibold"
            :class="destino.kind === 'conversa' ? 'bg-blue-100 text-blue-700' : 'bg-emerald-100 text-emerald-700'"
          >
